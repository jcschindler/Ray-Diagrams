...
### got rid of lines which hit top/bottom of lens (note: these lines may be important too, but I don't account for internal reflections yet.)
...
class light:
    def __init__(self,lens,distance,number_of_rays,dist_offset,height_offset):
...
def run(lens,light,x_lens,INDEX_AIR):
    ...
    plt.title('X Offset = ' + format(light.offset[0],'.2f')+ '    Y Offset = '+ format(light.offset[1],'.2f'))
    plt.show()
#
...
# for i in range(9):
#     lamp2 = light(lens2,distance=.723,number_of_rays=21,dist_offset=-0.1*i,height_offset=0.1*i)
#     run(lens2,lamp2,x_lens2,INDEX_AIR)
# i = 10
lamp2 = light(lens2,distance=.723,number_of_rays=21,dist_offset=0,height_offset=0)
